    ...
    )
    {
        ...
        if (created > modified)
            throw new ArgumentException(
                "Дата создания не может быть больше даты изменения",
                nameof(created)
            );
        if (modified < created)
            throw new ArgumentException(
                "Дата изменения не может быть меньше даты создания",
                nameof(modified)
            );
        ...
        switch (login.Length)
        {
            case < EntityConstraints.User.LoginMinLength:
                throw new ArgumentException(
                    $"Логин должен содержать минимум {EntityConstraints.User.LoginMinLength} символа",
                    nameof(login)
                );
            case > EntityConstraints.User.LoginMaxLength:
                throw new ArgumentException(
                    $"Логин может содержать максимум {EntityConstraints.User.LoginMaxLength} символа",
                    nameof(login)
                );
        }
        ...
        switch (firstName.Length)
        {
            case < EntityConstraints.User.FirstNameMinLength:
                throw new ArgumentException(
                    $"Имя должно содержать минимум {EntityConstraints.User.FirstNameMinLength} символ",
                    nameof(firstName)
                );
            case > EntityConstraints.User.FirstNameMaxLength:
                throw new ArgumentException(
                    $"Имя может содержать максимум {EntityConstraints.User.FirstNameMaxLength} символа",
                    nameof(firstName)
                );
        }
        ...
        switch (lastName.Length)
        {
            case < EntityConstraints.User.LastNameMinLength:
                throw new ArgumentException(
                    $"Фамилия должна содержать минимум {EntityConstraints.User.LastNameMinLength} символ",
                    nameof(lastName)
                );
            case > EntityConstraints.User.LastNameMaxLength:
                throw new ArgumentException(
                    $"Фамилия может содержать максимум {EntityConstraints.User.LastNameMaxLength} символа",
                    nameof(lastName)
                );
        }
        ...
        if (email.Length > EntityConstraints.User.EmailMaxLength)
            throw new ArgumentException(
                $"Email может содержать максимум {EntityConstraints.User.EmailMaxLength} символов",
                nameof(email)
            );
        ...
        if (
            ...
            || passwordHash.Length < EntityConstraints.User.PasswordMinLength
        )
            throw new ArgumentException(
                $"Хэш пароля должен содержать минимум {EntityConstraints.User.PasswordMinLength} символов",
                nameof(passwordHash)
            );
        ...
        if (
            ...
            && !Uri.TryCreate(profileImage, UriKind.Absolute, out _)
        )
            throw new ArgumentException(
                "Некорректный формат ссылки на изображение",
                nameof(profileImage)
            );
        if (profileImage.Length > EntityConstraints.User.ProfileImageMaxLength)
            throw new ArgumentException(
                $"Ссылка на изображение может содержать максимум {EntityConstraints.User.ProfileImageMaxLength} символов",
                nameof(profileImage)
            );
    ...
    public class UserBuilder
    {
        ...
        public UserEntity Build()
        {
            ...
            if (
                ...
                || _passwordHash.Length < EntityConstraints.User.PasswordMinLength
            )
                throw new ArgumentException(
                    $"Хэш пароля должен содержать минимум {EntityConstraints.User.PasswordMinLength} символов",
                    nameof(_passwordHash)
                );
            if (_profileImage.Length > EntityConstraints.User.ProfileImageMaxLength)
                throw new ArgumentException(
                    $"Ссылка на изображение может содержать максимум {EntityConstraints.User.ProfileImageMaxLength} символов",
                    nameof(_profileImage)
                );
